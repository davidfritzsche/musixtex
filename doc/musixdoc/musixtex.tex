...
% which enables >12 elements.  Search the string "T115" for details.
%
...
% \let\maxinstruments\si@ % [T115] here disabled to avoid conflict
...
% [T115] moved out to the new register allocation code -- start from here --
% altitude
%  \newskip\k@ii
% InterNote
%  \newdimen\i@ni
...
% [T115] moved out to the new register allocation code -- until here --
% \let\maxgroups\thr@@ % maximum three groups of instruments or choirs [T115] moved to later, using \setmaxgroups
\newcount\group@no

%%% [T115] new register allocation code -- start from here --
%
...
%
%%% [T115] new register allocation code -- until here --
...
%\ifx\undefined\sevenrm   % T117 RDT  unconditional font definitions
  \font\sevenrm=cmr7
  \font\sevenbf=cmbx7
  \font\sevenit=cmti7
%\fi
...
\font\ppfftwelve=cmbxti10 at 8pt  % T117 RDT
...
% [T115] moved out to the new register allocation code (except for \s@Y)
% horizontal start position of slur
%  \newdimen\s@xi
% sense u,d and x (flag) for pending slurs
%  \newtoks\s@si  \s@si={x}
...
%
% [T115] make slurs independent from \maxinstruments
\def\test@slurnum{%
  \ifnum\n@i<\z@ \n@i\@c \fi
  \ifnum\n@i<\maxslurs \else
...
\def\s@l@ctslur#1\relax{%
  \n@i#1\relax\test@slurnum% [T115] make it independent from beams
...
\def\comput@fonts{%    
  \ifnum\musicsize=\sixt@@n
    ...
    \let\tinyppff\ppfftwelve   % T117  RDT
  ...
  \else\ifnum\musicsize=\@xxiv
    ...
    \let\tinyppff\ppffsixteen  % T117  RDT
  ...
  \else\ifnum\musicsize=\@xxix
    ...
    \let\meterLargefont\twfvbf
    \let\tinyppff\ppfftwenty   % T117  RDT
...
% [T115] moved out to the new register allocation code
...
\def\s@l@ctinstr{%
  \ifnum\noinstrum@nt>\maxinstruments % [T115] -- from here --
...
\def\test@grnum{%
  ...
  \ifnum\group@no<\maxgroups \else
    ...
    \errmessage{Wrong group reference number \the\group@no! (valid: 0 to \the\count@)}% [T115]
    \group@no\z@% [T115]
  \fi
  \advance\group@no\@ne}
...
% [T115] moved out to the above of the new register allocation code
...
% cutslur
  \ifnum\N@s>\z@%     % any pending slurs ?
    \n@viii\maxslurs % test all possible slur numbers [T115] (Hiroaki)
    \advance\n@viii\m@ne
    \loop\ifnum\n@viii>\m@ne
      \s@l@ctslur\n@viii\relax
      \edef\@sense{\the\s@s}%
...
% [T115] reqest from Hermann Hinsch
...
% [T115] reqest from Hermann Hinsch: end
...
%  \let\Writ@names\writ@names
  \expandafter\def\csname instrument\romannumeral\n@v\endcsname{#2}}% [T115] changed from \edef to \def (Hiroaki)
...
\def\tqqh#1#2{\tbbu{#1}\tqh{#1}{#2}} % T117 RDT
\def\tqqu#1#2{\tbbu{#1}\tqu{#1}{#2}} % T117 RDT
\def\tqqb#1#2{\tbbl{#1}\tqb{#1}{#2}} % T117 RDT
\def\tqql#1#2{\tbbl{#1}\tql{#1}{#2}} % T117 RDT
\def\tqqqh#1#2{\tbbbu{#1}\tqqh{#1}{#2}} % T117 RDT
\def\tqqqu#1#2{\tbbbu{#1}\tqqu{#1}{#2}} % T117 RDT
\def\tqqqb#1#2{\tbbbl{#1}\tqqb{#1}{#2}} % T117 RDT
\def\tqqql#1#2{\tbbbl{#1}\tqql{#1}{#2}} % T117 RDT
...
\def\test@beamnum{% [T115] new macro to make it independent from slur
  \ifnum\n@i<\z@ \n@i\@c \fi
...
\def\s@l@ctbeam#1\relax{%
  \n@i#1\relax\test@beamnum % [T115] make it independent from slur
...
% \n@ii positive signature limit, \n@ii-8 negative signature limit
    \advance\n@ii-\a@s \advance\n@ii\si@ \divide\n@ii\s@v@n
    \@Tii
  \fi \advance\n@v\@l \advance\n@v\n@vi \expandafter\@Ti % [T115] (Hiroaki)
  \fi}
...
\def\octnumberdefault{\smallppff8}  % T117 RDT
...
\def\triolet{\xtuplet\txt}    % T117 RDT
...
\def\nnotes{\vnotes1.6\elemskip}    % T116  RDT (corrected 2015-03-31)
\def\nnnotes{\vnotes1.28\elemskip}  % T116  RDT (corrected 2015-03-31)
...
% [T115] setup the default maximum element numbers to the same values of T.114
...
% [T115] new register allocation code requires some changes
%\resetlayout % no need
